...
/**
 * User agent generator and parser for Instagram Android
 *
...
class IgAgent
{
     ...
     * @var string
     */
    private string $manufacturerWithBrand;
    /**
     * @var string
     */
    ...
    public function __construct(string $userAgent = null)
    {
        if (empty($userAgent)) {
            ...
            $this->deviceString = $devices[array_rand($devices)];
            $this->igVersion = "151.0.0.23.120";
            $this->igBuild = "232868034";
            $this->locale = "en_US";
        } else {
            $this->deviceString = $this->findBetween($userAgent, "(", ")");
            if (empty($this->deviceString))
                throw new Exception("User agent is invalid: deviceString");
        }

        $parts = array_map("trim", explode(";", $this->deviceString));
        if (count($parts) < 7)
            throw new Exception("Device string is invalid: partsCount");
        $android = explode("/", $parts[0], 2);
        if (empty($android[0]) || empty($android[1]))
            throw new Exception("Device string is invalid: android");

        if (!empty($userAgent)) {
            $this->igVersion = $this->findBetween($userAgent, "Instagram", "Android");
            if (empty($this->igVersion))
                throw new Exception("User agent is invalid: igVersion");
            $this->igBuild = $parts[8] ?? "";
            if (empty($this->igBuild))
                throw new Exception("User agent is invalid: igBuild");
            $this->locale = $parts[7] ?? "";
            if (empty($this->locale))
                throw new Exception("User agent is invalid: locale");
        }

        try {
            $this->manufacturerWithBrand = $parts[3];
            $manufacturerAndBrand = explode("/", $this->manufacturerWithBrand, 2);
            $this->androidRelease = $android[0];
            $this->androidVersion = $android[1];
            $this->dpi = $parts[1];
            $this->resolution = $parts[2];
            $this->model = $parts[4];
            $this->device = $parts[5];
            $this->cpu = $parts[6];
            $this->manufacturer = $manufacturerAndBrand[0];
            $this->brand = $manufacturerAndBrand[1] ?? "";
        } catch (Exception  $e) {
            throw new Exception("Device string is invalid: partsAll");
        }
    ...
    {
        return sprintf(
            'Instagram %s Android (%s/%s; %s; %s; %s; %s; %s; %s; %s; %s)',
            $this->igVersion,
            $this->androidRelease,
            $this->androidVersion,
            $this->dpi,
            $this->resolution,
            $this->manufacturerWithBrand,
            $this->model,
            $this->device,
            $this->cpu,
            $this->locale,
    ...
    /**
     * @param string $igVersion
     */
    public function setIgVersion(string $igVersion): void
    {
        $this->igVersion = $igVersion;
    }

    /**
     * @param string $igBuild
     */
    public function setIgBuild(string $igBuild): void
    {
        $this->igBuild = $igBuild;
    }
     ...
     * @return string
     */
    public function getManufacturerWithBrand(): string
    {
        return $this->manufacturerWithBrand;
    }
     ...
     * @return string
     */
    public function getIgBuild(): string
    {
        return $this->igBuild;
    }
     ...
     * @return string
     */
    private function findBetween(string $string, $start, $end): string
    {
